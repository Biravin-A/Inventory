...
def predict():
    try:
        ...
        model_columns = ['invoice_day', 'invoice_month', 'invoice_year', 'current_km_reading', 
                         'business_partner_name_X', 'vehicle_model_Y', 'days_diff']  # Replace with actual columns from training
        ...
        return render_template('index.html', prediction_text=f"Error: {str(e)}. Please # main.py
...
if __name__ == '__main__':
    app.run(debug=True)gunicorn main:app --bind 0.0.0.0:8000ensure all fields are filled out correctly.")

    except KeyError as e:
        # Handle missing columns in the DataFrame (in case get_dummies() doesn't generate the right columns)
        return render_template('index.html', prediction_text=f"Error: Missing column for {str(e)}. Please check input data.")
